...
class App extends Component {
  ...
  imageClick = thumbnail => {
    const {imgUrl} = this.state
    const imageValue = imagesList.filter(
      eachImg => eachImg.thumbnailUrl === thumbnail,
    )
  ...
  updatedActiveTabId = tabValue => {
    this.setState({activeTabId: tabValue})
  }

  render() {
    const {time, score, imgUrl, activeTabId, isActive} = this.state

    const filterImagesList = imagesList.filter(
      eachImages => eachImages.category === activeTabId,
    )
          ...
          {!isActive && (
            <div className="first-div">
              ...
              <ul className="thumbnail-images">
                {filterImagesList.map(eachImg => (
                  <ProjectItem
                    imageDetails={eachImg}
                    key={eachImg.id}
                    imageClicking={this.imageClick}
                  />
                ))}
              </ul>
            </div>
